...
Click Install to begin the installation. If you want to change your installation settings, click Back." \
    ...
    for feature_key in ${feature_keys} ; do
        case "${feature_key}" in
            "rdr")
                packages="${packages} cprocsp-rdr-pcsc cprocsp-rdr-emv cprocsp-rdr-inpaspot cprocsp-rdr-kst cprocsp-rdr-mskey cprocsp-rdr-novacard cprocsp-rdr-rutoken cprocsp-rdr-cloud cprocsp-rdr-cpfkc cprocsp-rdr-infocrypt cprocsp-rdr-rosan"
                ;;
            "ssl")
...
Click on the list items below to change the way features will be installed." \
            ...
            "cprocsp-rdr-gui-gtk" "GUI for smart card and token support modules" "ON" \
            "rdr" "Smart Card and Token support modules" "ON" \
            "cprocsp-cptools-gtk" "cptools, GUI application for various CSP tasks" "ON" \
            "ssl" "OpenSSL library" "OFF" \
            "cprocsp-stunnel" "stunnel, SSL/TLS tunnel with GOST support" "ON" \
            "lsb-cprocsp-pkcs11" "PKCS #11 library" "ON" \
            3>&1 1>&2 2>&3
    )"
    if [ "$?" -ne "${SUCCESS}" ] ; then
        exit "${SUCCESS}"
        ...
        for feature_key in ${feature_keys} ; do
            case "${feature_key}" in
                ...
                "rdr")
                    ...
                    ;;
                "cprocsp-cptools-gtk")
                    description="cptools, GUI application for various CSP tasks"
                    ;;
                "ssl")
